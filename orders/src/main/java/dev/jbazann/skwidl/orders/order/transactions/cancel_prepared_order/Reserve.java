    ...
    public TransactionResult runStage(DomainEvent domainEvent,
        ...
        if (STATUS == StatusHistory.Status.CANCELED) {
            ...
            return new TransactionResult()
                    ...
                    .setContext("Order was already canceled.");
        }
        ...
        if(STATUS != StatusHistory.Status.ACCEPTED) {
            ...
            return new TransactionResult()
                    .setData(transaction)
                    .setSimpleResult(TransactionResult.SimpleResult.FAILURE)
                    .setContext("Order status was expected to be 'accepted', but is instead " + STATUS + '.');
        }

        // TODO transaction.
        orderActions.cancel(order, "Canceled by transaction id: "+event.getTransaction().getId());
        transaction = transactionActions.accept(transaction);
        return new TransactionResult()
                .setData(transaction)
                .setSimpleResult(TransactionResult.SimpleResult.SUCCESS)
                .setContext("Order gracefully canceled.");
